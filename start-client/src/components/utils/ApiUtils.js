...
export const postRun = function postRun(url, request) {
  return new Promise((resolve, reject) => {
    fetch(`${url}`, {
      method: 'POST',
      headers: {
...
export const getProjectParams = function getProjectParams(values, config) {
  ...
  get(values, 'dependencies', [])
      ...
      .filter(dep => !!dep)
      .map(dep => paramsDependencies.push(dep))
  return {
    ...
    'dependencies': paramsDependencies
  }
}
