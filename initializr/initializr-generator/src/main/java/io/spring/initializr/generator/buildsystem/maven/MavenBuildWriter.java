...
public class MavenBuildWriter {
    ...
    public void writeRootPom(IndentingWriter writer, MavenBuild build) {
        MavenBuildSettings settings = build.getSettings();
        writeProject(writer, () -> {
            writeProjectCoordinates(writer, settings);
            writeSingleElement(writer, "packaging", "pom");
            writeProjectName(writer, settings);
            writeParent(writer, build);
    ...
    private void writeModules(IndentingWriter writer, MavenBuild build) {
        List<String> modules = build.getSettings().getModules();
        if (modules == null || modules.isEmpty()) {
            return;
        }
        writer.println("<modules>");
        for (String module : modules) {
            writer.indented(() -> writeSingleElement(writer, "module", module));
        }
        writer.println("</modules>");
    }
